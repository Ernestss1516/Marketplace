...
test.describe('Páginas informativas — /admin/paginas y /paginas/[slug]', () => {
  test('ADMIN crea y publica una página; se sirve en /paginas/[slug] con presentación de página y no aparece en el feed del blog', async ({
    ...
    await editorTextarea.fill('# Encabezado\n\nContenido de la página con **negrita**.\n');
    ...
    const pageId = editUrl.match(/\/admin\/paginas\/([^/]+)\/editar/)?.[1];
    ...
    await expect(popup.getByText('Encabezado')).toBeVisible();
    await expect(popup.getByText('negrita')).toBeVisible();
  ...
  test('un <script> literal en una página nunca se ejecuta (regla invariante de seguridad)', async ({
    adminContext,
  }) => {
    const page = await adminContext.newPage();
    await page.goto('/admin/paginas/nueva');
    ...
    await expect(editorTextarea).toBeVisible();
    await editorTextarea.click();
    await editorTextarea.fill('<script>window.__xss_executed = true;</script>\n');
