...
export default function EditarPaginaPage() {
  ...
  const fetchPost = useCallback(async () => {
    ...
    try {
      ...
      setLoadError(mensajeDeErrorAdmin(err, 'Error al cargar la página'));
    }
  }, [token, id]);
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3">
          <Link href="/admin/paginas" className="shrink-0 text-sm text-muted-foreground hover:underline">
            ← Páginas
          </Link>
        ...
        <div className="flex shrink-0 items-center gap-2">
          {post.status === 'PUBLISHED' && (
            <Button variant="outline" size="sm" asChild>
              <Link href={`/paginas/${post.slug}`} target="_blank">
                Ver página ↗
              </Link>
            </Button>
          )}
