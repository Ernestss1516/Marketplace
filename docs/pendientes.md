...
| **Sistema de estilo — E8: los correos** (2026-09-05) | La última pieza, y la única que cruzaba una invariante de seguridad — **aprobada por Ernest** (decisión #3). Los correos llevan HTML temado y la invariante NO se elimina: se traslada de «nunca hay HTML» a «el HTML se compone en un solo sitio y todo dato entra escapado, siempre». Lo que lo sostiene es de TIPOS, no de disciplina: los 18 métodos entregan **piezas tipadas** donde no cabe marcado, y el serializador usa una plantilla que **no permite expresar HTML sin escapar** — no hay `crudo()` que exportar. El admin no es campo privilegiado: su `cuerpo` y su `motivo` se escapan igual. Del tema llegan principal, rampa neutra y logo, en hexadecimal inline; **la tipografía no** — la personalidad en el correo es parcial, y está aceptado. Detalle en [`diseno-sistema-estilo.md`](./diseno-sistema-estilo.md) §7.7 | (esta ráfaga) |
