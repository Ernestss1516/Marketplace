...
| **Favorites** | ✅ Completo | `POST /favorites/:listingId` (marcar), `DELETE /favorites/:listingId` (desmarcar), `GET /favorites` (paginado), `GET /favorites/:listingId` (check). Todos idempotentes y con `JwtAuthGuard`. Suite `favorites.e2e-spec.ts` (9 tests) |
...
| **Favoritos** `/favoritos` | ✅ Completo | Ruta protegida. SSR paginado; grid de `ListingCard`; estado vacío con enlace a búsqueda. Botón corazón en ficha de anuncio (client component, check en mount, toggle optimista, solo visible con sesión) |
