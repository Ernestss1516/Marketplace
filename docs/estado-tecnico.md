...
> Fecha: 2026-07-01 · Rama: `main` · Último commit: RL5.1-C — avatar de perfil subible: POST /media/upload-avatar + AvatarUpload en PerfilForm + 2 tests Playwright (avatar-upload.spec.ts)
...
| **Búsqueda — vista de mapa (básica)** | ✅ Completo (H6.5a) | Toggle `?view=mapa` en `/busqueda`: botones Lista/Mapa en la cabecera. `MapView` client-only (`dynamic({ ssr: false })`) con MapLibre GL JS + tiles vectoriales MapTiler `streets-v2`. Markers individuales por hit con `_geo` válido; `fitBounds` ajusta el viewport. Modo mapa: `hitsPerPage=200` sin paginación; modo lista: 24 con paginación normal. `_geo?: { lat, lng }` en `ListingSummary`. `'view'` en `KNOWN_PARAMS`. DTO `@Max` subido a 200. Skeleton `animate-pulse` durante carga del bundle. Tests: `busqueda-mapa.spec.ts` (5 casos). H6.5b (clustering + panel detalle + aviso sin geo): pendiente. **Verificación manual requerida**: tiles de MapTiler cargando, markers en la ciudad correcta (no lat/lng invertido). |
