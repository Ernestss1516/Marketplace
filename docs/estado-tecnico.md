...
### H6.4 — Rediseño portada (diferido → Hito 9)

H6.4 (rediseño visual de la portada `/`) fue diferido conscientemente: el trabajo de H6.5 (mapa) agotó la ráfaga de Hito 6 enfocado. No hay deuda técnica; es backlog de UI. Pendiente para **Hito 9 (pulido UI)**.

### H6.6 — Anuncios patrocinados (diferido → Hito 8 o según decisión de negocio)

H6.6 fue diferido: requiere decisión sobre si los patrocinados son de pago (Hito 8, integración billing) o solo admin sin cobro (antes). El diseño de H6.1 queda documentado para no rehacer el análisis:

- **Modelo `SponsoredAd`** (pendiente de migración Prisma): `id`, `categorySlugs: String[]`, `startsAt: DateTime`, `endsAt: DateTime`, `targetUrl: String`, más campos de tracking. Solo admin backoffice (sin checkout en esta fase).
- **Presentación**: badge "Publicidad" (distinto de "Destacado") superpuesto en la tarjeta.
