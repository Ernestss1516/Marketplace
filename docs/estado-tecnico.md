...
| **RedsysModule** | ⚠️ RF.5 Lógica verificada, integración pendiente | `RedsysService` (generación de form + Ds_Order YYYYMMDD+4random con retry), `RedsysWebhookGuard` (HMAC vía `redsys-easy`, idempotencia por `GatewayEvent`), `RedsysProcessor` (acreditación wallet atómica + validación de importe). Endpoints: `POST /billing/checkout/credits-pack`, `POST /billing/checkout/featured-pay`, `POST /webhooks/redsys`. **Verificado e2e**: acreditación wallet (12 tests: IVA, Ds_Order uniqueness, idempotencia ×2, validación importe, edge cases). **Pendiente de verificación**: firma HMAC real y notificación online de Redsys (requiere tooling: túnel público + credenciales sandbox). `featuredByRedsys` (destacado directo) implementado en guard/processor con TODO — se completa en RF.6 cuando exista `grantFeaturedListing` |
